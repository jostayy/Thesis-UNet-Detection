...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, BatchNormalization, Activation, RandomFlip, RandomRotation
...
SAVE_DIR = os.path.join(config.FIGURE_DROP_PATH, "Eksperyment_SegNet")
os.makedirs(SAVE_DIR, exist_ok=True)
BEST_MODEL_PATH = os.path.join(SAVE_DIR, "segnet_best_model.h5")
...
if __name__ == "__main__":
    ...
    model = build_segnet((IMG_SHAPE[0], IMG_SHAPE[1], NUM_CHANNELS))
    ...
    plt.title('Training History - SegNet')
    plt.legend()
    ...
    for i in range(len(preds)):
        ...
        ax[2].set_title(f'Prediction (Dice: {dice:.3f})')
        ax[2].axis('off')
